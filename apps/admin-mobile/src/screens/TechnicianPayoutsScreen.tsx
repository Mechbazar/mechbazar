...
// Mirrors apps/admin/src/pages/services/TechnicianPayouts.tsx's list;
// detail/process actions live on the pushed TechnicianPayoutDetailScreen.
export const TechnicianPayoutsScreen = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return settlements.filter((s: any) => s.technician?.user?.name?.toLowerCase().includes(q) || s.technician?.user?.phone?.includes(q));
  }, [settlements, search]);
  ...
  return (
    <View style={styles.container}>
      <Typography variant="h2" style={{ padding: 16, paddingBottom: 8 }}>Technician Payouts</Typography>

      <View style={{ paddingHorizontal: 16 }}>
        <Input placeholder="Search technician, phone..." value={search} onChangeText={setSearch} />
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity onPress={() => navigation.navigate('TechnicianPayoutDetail', { settlementId: item.id })}>
              <Card style={styles.card}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <View>
                    <Typography variant="h3">{item.technician?.user?.name || 'Unknown Technician'}</Typography>
                    <Typography variant="caption">{item.technician?.user?.phone}</Typography>
                  </View>
