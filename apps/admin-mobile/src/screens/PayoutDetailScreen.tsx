...
// Mirrors apps/admin/src/pages/Payouts.tsx's "Process Payout" modal:
// transactionId is required before "Mark Completed" can be pressed; non-
// PENDING settlements are read-only.
export const PayoutDetailScreen = () => {
  ...
  const statusMutation = useMutation({
    mutationFn: (payload: { status: string; transactionId?: string }) => adminService.updateSettlementStatus(settlementId, payload),
    onSuccess: () => {
  ...
  const bankAccount = settlement.vendor?.bankAccounts?.[0];
  ...
  const handleFail = () => {
    Alert.alert('Fail & Refund', 'This will refund the amount to the vendor wallet. Continue?', [
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
      <Card>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <Typography variant="h3">{settlement.vendor?.storeName}</Typography>
