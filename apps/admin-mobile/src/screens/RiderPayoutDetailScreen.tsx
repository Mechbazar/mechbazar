...
// Mirrors apps/admin/src/pages/RiderPayouts.tsx's "Process Payout" modal.
export const RiderPayoutDetailScreen = () => {
  ...
  const statusMutation = useMutation({
    mutationFn: (payload: { status: string; transactionId?: string }) => adminService.updateRiderSettlementStatus(settlementId, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-rider-settlements'] });
      navigation.goBack();
    },
  ...
  const bankAccount = settlement.deliveryPartner?.bankAccounts?.[0];
  ...
  const handleFail = () => {
    Alert.alert('Fail & Refund', 'This will refund the amount to the rider wallet. Continue?', [
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
      <Card>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <Typography variant="h3">{settlement.deliveryPartner?.user?.name || 'Unknown Rider'}</Typography>
