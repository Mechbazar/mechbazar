...
import { colors, Typography, Card, Loader, vendorService, NotificationItem } from '@mechbazar/shared';
...
export const NotificationsScreen = () => {
  ...
  const load = useCallback(
    async (reset: boolean) => {
      reset ? setLoading(true) : setFetchingMore(true);
      try {
        const page = await vendorService.getNotificationsPage({
          limit: 20,
          ...(reset ? {} : cursor ? { cursor } : {}),
          ...(debouncedSearch.trim() ? { q: debouncedSearch.trim() } : {}),
          ...(category !== 'ALL' ? { category } : {}),
        });
        setPages((prev) => (reset ? page.items : [...prev, ...page.items]));
    ...
    [cursor, debouncedSearch, category]
  );
  ...
  const markRead = async (id: string) => {
    ...
    setPages((prev) => prev.map((n) => (n.id === id ? { ...n, isRead: true } : n)));
  };

  const markAllRead = async () => {
    await vendorService.markAllNotificationsRead();
    setPages((prev) => prev.map((n) => ({ ...n, isRead: true })));
  };

  const remove = async (id: string) => {
    await vendorService.deleteNotification(id);
    setPages((prev) => prev.filter((n) => n.id !== id));
  };

  const openNotification = (item: NotificationItem) => {
    if (!item.isRead) markRead(item.id);
    vendorService.markNotificationOpened(item.id).catch(() => {});
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        onEndReached={() => cursor && !fetchingMore && load(false)}
