...
// Firebase Phone Auth for the seller app (native-only -- this app ships as an
...
// verifies (see apps/backend/src/utils/otp.ts). Byte-for-byte mirror of
// apps/rider/src/services/phoneAuth.ts (and apps/mechanic's copy) apart from
// the log tag -- same auto-verification/session-expired handling applies here
// since this is the same RNFB Android SDK.
//
...
const log = (msg: string, extra?: unknown) => {
  const line = `[otp:seller] ${new Date().toISOString()} ${msg}`;
  if (extra !== undefined) {
 ...
 * verified -- an ID token for a different number would log the wrong seller in.
 */
...
// Resolves to a Firebase ID token, sent as the `otp` field to /vendors/register
// and /vendors/login, which verify it via firebase-admin.
