    ...
    async (idToken: string) => {
      ...
      try {
        const data = mode === 'login'
          ? await riderService.login({ phone: phone.replace(/\D/g, '').slice(-10), otp: idToken })
          : await riderService.register({ phone: phone.replace(/\D/g, '').slice(-10), otp: idToken, name: name.trim(), email: email.trim() || undefined });
