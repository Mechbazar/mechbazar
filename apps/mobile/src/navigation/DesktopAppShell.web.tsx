...
import { useDesktopHomeFocused } from './desktopHomeFocusStore';
...
export default function DesktopAppShell({ children }: { children: React.ReactNode }) {
  ...
  const isHomeActive = useDesktopHomeFocused();
  ...
  if (isHomeActive) {
    ...
    // appearing after scrolling through a long page. HomeScreenDesktop owns
    // its own full-height ScrollView with a DesktopFooter as its last
    // element instead, so this shell steps out of the way rather than
    // double-wrapping.
    return (
