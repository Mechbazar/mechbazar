...
type DocType = 'AADHAAR' | 'DRIVING_LICENSE';

// DRIVING_LICENSE is optional -- not in the backend's REQUIRED_DOCUMENT_TYPES
// (only AADHAAR + SELFIE gate submission), so it's just an extra document an
// admin can review if uploaded, never a blocker for this wizard.
const DOCUMENT_TYPES: { key: DocType; label: string }[] = [
  { key: 'AADHAAR', label: 'Aadhaar Card' },
  { key: 'DRIVING_LICENSE', label: 'Driving License (optional)' },
];
...
const STEP_TITLES = ['Personal & Address', 'Specializations & Skills', 'Documents', 'Bank Account', 'Emergency Contact', 'Selfie', 'Review & Submit'];
...
// submitted incrementally to PATCH /technicians/me/registration.
export const RegistrationScreen = () => {
  const queryClient = useQueryClient();
  const [step, setStep] = useState(0);

  const { data: profile, isLoading } = useQuery({ queryKey: ['technician-profile'], queryFn: technicianService.getProfile });

  const [form, setForm] = useState({
    addressLine: '', city: '', state: '', pincode: '', aadhaarNumber: '',
    specializations: [] as string[], skills: [] as string[], experienceYears: '',
  ...
  const registrationMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => technicianService.updateRegistration(data),
  ...
  const documentMutation = useMutation({
    mutationFn: ({ type, uri }: { type: string; uri: string }) =>
      technicianService.uploadDocument(type, uri, 'image/jpeg', `${type}-${Date.now()}.jpg`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['technician-profile'] }),
    onError: (err: any) => Alert.alert('Upload failed', err.response?.data?.error || err.message),
  });

  const submitMutation = useMutation({
    mutationFn: () => technicianService.submitForApproval(),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['technician-profile'] }),
    onError: (err: any) => {
  ...
  const goNext = async () => {
    if (step === 0) {
      ...
      if (form.specializations.length === 0) {
        Alert.alert('Missing details', 'Select at least one specialization (Car or Bike).');
        return;
      }
      await registrationMutation.mutateAsync({
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
      <Typography variant="h2">Mechanic Registration</Typography>
      ...
      <Card style={{ marginTop: 16 }}>
        ...
        {step === 1 && (
          <View style={{ gap: 12 }}>
            <Typography variant="body" style={{ fontWeight: '600' }}>Specializations</Typography>
            <View style={{ flexDirection: 'row', gap: 8 }}>
              {SPECIALIZATIONS.map(({ key, label }) => (
                <TouchableOpacity
                  key={key}
                  onPress={() => toggleSpecialization(key)}
                  style={[styles.chip, form.specializations.includes(key) && styles.chipActive]}
                >
                  <Typography variant="caption" style={{ color: form.specializations.includes(key) ? '#ffffff' : colors.text }}>{label}</Typography>
                </TouchableOpacity>
              ))}
            </View>
            <Typography variant="body" style={{ fontWeight: '600', marginTop: 8 }}>Skills</Typography>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {SKILL_CATALOG.map((skill) => (
                ...
                >
                  <Typography variant="caption" style={{ color: form.skills.includes(skill) ? '#ffffff' : colors.text }}>{skill}</Typography>
                </TouchableOpacity>
              ))}
            </View>
            <Input
              ...
              onChangeText={(v) => setForm({ ...form, experienceYears: v.replace(/\D/g, '') })}
            />
          </View>
        )}
