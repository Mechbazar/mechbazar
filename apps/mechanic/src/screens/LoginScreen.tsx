...
import { colors, Button, Typography, Input, technicianService, getApiBaseUrl } from '@mechbazar/shared';
import { Wrench } from 'lucide-react-native';
import { setAuth } from '../store';

// Technicians are created by admin without a password, so they log in the
// same way the phone+OTP flow works for every role today: the backend
// accepts a dev-bypass OTP of '123456' (no real Firebase SMS verification
// wired up here yet). New technicians can also self-register here (the only
// entry point into the KYC wizard, see RootNavigator).
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = mode === 'login'
        ? await technicianService.login({ phone: normalizedPhone, otp: normalizedOtp })
        : await technicianService.register({ phone: normalizedPhone, otp: normalizedOtp, name: name.trim(), email: email.trim() || undefined });
    ...
    <View style={{ flex: 1, justifyContent: 'center', padding: 24, backgroundColor: '#ffffff' }}>
      <View style={{ alignItems: 'center', marginBottom: 48 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          >
            <Wrench color={colors.primary} size={26} />
          </View>
