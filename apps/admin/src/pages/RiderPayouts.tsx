...
export default function RiderPayouts() {
  ...
  const fetchSettlements = async () => {
    try {
      const res = await axios.get(`${API_URL}/riders/settlements`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setSettlements(res.data);
    } catch (err) {
      console.error('Error fetching rider settlements:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateStatus = async (status: string) => {
    ...
    try {
      await axios.patch(`${API_URL}/riders/settlements/${selectedSettlement.id}/status`,
  ...
  const filteredSettlements = settlements.filter(s => {
    if (searchTerm) {
      ...
      return (s.deliveryPartner?.user?.name && s.deliveryPartner.user.name.toLowerCase().includes(term)) ||
             (s.deliveryPartner?.user?.phone && s.deliveryPartner.user.phone.includes(term));
    }
    return true;
  });
      ...
      <div className="bg-neutral-900 rounded-3xl border border-neutral-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-neutral-800 flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-neutral-950">
          <div>
            ...
            <p className="text-neutral-400 text-sm mt-1">Review recent rider settlements and update payout status.</p>
          </div>
            ...
            <tbody className="divide-y divide-neutral-800">
              {filteredSettlements.map((s: any) => (
                <tr key={s.id} className="hover:bg-neutral-900 transition-all duration-200 cursor-pointer group">
                  ...
                  <td className="p-4">
                    <div className="text-white font-bold">{s.deliveryPartner?.user?.name || 'Unknown Rider'}</div>
                    <div className="text-xs text-neutral-500">{s.deliveryPartner?.user?.phone}</div>
                  </td>
                  <td className="p-4">
      ...
      {selectedSettlement && (
        ...
        >
              ...
              <div className="mb-6 bg-neutral-900 p-4 rounded-2xl border border-neutral-800">
                ...
                {selectedSettlement.deliveryPartner?.bankAccounts && selectedSettlement.deliveryPartner.bankAccounts.length > 0 ? (
                  <div className="space-y-3">
                    <div className="flex justify-between text-neutral-400">
                      <span>Bank Name</span>
                      <span className="font-bold text-white">{selectedSettlement.deliveryPartner.bankAccounts[0].bankName}</span>
                    </div>
                    <div className="flex justify-between text-neutral-400">
                      <span>Account Holder</span>
                      <span className="font-bold text-white">{selectedSettlement.deliveryPartner.bankAccounts[0].accountHolderName}</span>
                    </div>
                    <div className="flex justify-between text-neutral-400">
                      <span>Account No.</span>
                      <span className="font-bold text-white font-mono">{selectedSettlement.deliveryPartner.bankAccounts[0].accountNumber}</span>
                    </div>
                    <div className="flex justify-between text-neutral-400">
                      <span>IFSC Code</span>
                      <span className="font-bold text-white font-mono">{selectedSettlement.deliveryPartner.bankAccounts[0].ifscCode}</span>
                    </div>
                  </div>
                ) : (
                  <p className="text-danger-300 font-medium">No bank account found for this rider.</p>
                )}
              </div>
